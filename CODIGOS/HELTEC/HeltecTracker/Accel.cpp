...
void DetectorCaida::checkStatus(){
  if (isMonitoringActive) {  //Monitorear detector de caida solo si se está haciendo ejercicio
    if (!isCalibrated) { //Calibrar al momento de iniciar actividad fisica
      ...
      if ((activ.isInactivity && segunda_condicion_caida && !isPersonStanding()) || (activ.isInactivity && impacto && !isPersonStanding())) { emergencia = true; currentScreen = SCREEN_EMERGENCY; drawEmergencyScreen(true); currentScreen = SCREEN_MONITORING;}
