...
float Adafruit_ADXL345_Unified::getFreeFallThreshold(void) {
  return readRegister(ADXL345_REG_THRESH_FF) * 0.0625f;
}

float Adafruit_ADXL345_Unified::getFreeFallDuration(void) {
  return readRegister(ADXL345_REG_TIME_FF) * 0.005f;
}

void Adafruit_ADXL345_Unified::setFreeFallThreshold(float threshold)
{
    uint8_t scaled = constrain(threshold / 0.0625f, 0, 255);
    writeRegister(ADXL345_REG_THRESH_FF, scaled);
}

void Adafruit_ADXL345_Unified::setFreeFallDuration(float duration)
{
...
void Adafruit_ADXL345_Unified::setActivityXYZ(bool state, bool coupled) //coupled = 0 = dc, coupled = 1 = ac
{
    uint8_t value;
...
void Adafruit_ADXL345_Unified::setInactivityThreshold(float threshold)
{
    uint8_t scaled = constrain(threshold / 0.0625f, 0, 255);
...
float Adafruit_ADXL345_Unified::getInactivityThreshold(void)
{
    return readRegister(ADXL345_REG_THRESH_INACT) * 0.0625f;
}

// Set Inactivity Time (s / LSB)
void Adafruit_ADXL345_Unified::setTimeInactivity(uint8_t time)
{
    writeRegister(ADXL345_REG_TIME_INACT, time);
}

// Get Inactivity Time (s / LSB)
uint8_t Adafruit_ADXL345_Unified::getTimeInactivity(void)
{
    return readRegister(ADXL345_REG_TIME_INACT);
}

void Adafruit_ADXL345_Unified::setInactivityXYZ(bool state, bool coupled)
{
    uint8_t value;
...
void Adafruit_ADXL345_Unified::useInterrupt(adxl345_int_t interrupt)
{
    if (interrupt == 0)
    {
...
void Adafruit_ADXL345_Unified::setOffsets(int8_t x, int8_t y, int8_t z)
{
  writeRegister(ADXL345_REG_OFSX, x);
  writeRegister(ADXL345_REG_OFSY, y);
  writeRegister(ADXL345_REG_OFSZ, z);

}

Activites Adafruit_ADXL345_Unified::readActivites(void)
{
